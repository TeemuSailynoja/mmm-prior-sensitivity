...
def _():
    ...
    Before proceeding to the lift test, the modeling team reviewed the initial model and formed a hypothesis: the original baseline prior was slightly too flexible, absorbing some signal that should have been attributed to the media effects. We do not believe the baseline should be very flexible — we expect it to be mostly a slowly moving trend, not something that captures complex short-term patterns. If the baseline can represent too many functions, it can soak up patterns in the data that are actually driven by channel spend — and that would distort the channel estimates.

    To test this, we refit the business-prior model with a less expressive baseline: we reduce the number of HSGP basis functions from `m=100` to `m=50` and tighten the lengthscale prior from `ls_sigma=10` to `ls_sigma=5`. With fewer basis functions and a tighter lengthscale prior, the baseline can represent fewer functions and is less able to absorb short-term patterns. Any signal that the flexible baseline was previously soaking up should now be freed up and attributed to the media channels.
